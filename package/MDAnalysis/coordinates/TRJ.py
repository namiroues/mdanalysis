...
try:
    ...
    netCDF4 = None
    logger.warning(
        "netCDF4 is not available. Writing AMBER ncdf files will be slow."
    )
...
class TRJReader(base.ReaderBase):
    ...
    def __init__(self, filename, n_atoms=None, **kwargs):
        ...
        self.lines_per_frame = int(
            np.ceil(3.0 * self.n_atoms / len(self.default_line_parser))
        )
        ...
        self.last_line_parser = util.FORTRANReader(
            "{0:d}F8.3".format(last_per_line)
        )
    ...
    def _read_next_timestep(self):
        ...
        if self.periodic:
            ...
            ts.dimensions = box + [90.0, 90.0, 90.0]  # assumed
    ...
    def _detect_amber_box(self):
        ...
        if nentries == 3:
            self.periodic = True
            ts.dimensions = self.box_line_parser.read(line) + [
                90.0,
                90.0,
                90.0,
            ]
...
class NCDFReader(base.ReaderBase):
    ...
    *filename* is a file name, ``False`` when *filename* is a file-like object.
    ...
    def __init__(self, filename, n_atoms=None, mmap=None, **kwargs):
        ...
        self.trjfile = NCDFPicklable(
            self.filename, mmap=self._mmap, maskandscale=False
        )
        ...
        try:
            ...
            if not (
                ...
                or "AMBER" in conventions.decode("utf-8").split()
            ):
                errmsg = (
                    "NCDF trajectory {0} does not conform to AMBER "
                    "specifications, "
        ...
        try:
            ...
            if not ConventionVersion == self.version:
                wmsg = (
                    "NCDF trajectory format is {0!s} but the reader "
        ...
        if not (
            hasattr(self.trjfile, "program")
            and hasattr(self.trjfile, "programVersion")
        ):
            wmsg = (
                "NCDF trajectory {0} may not fully adhere to AMBER "
        ...
        try:
            ...
            errmsg = (
                "NCDF trajectory {0} does not contain atom "
        ...
        try:
            ...
            errmsg = (
                f"NCDF trajectory {self.filename} does not contain "
                f"frame information"
            )
            raise ValueError(errmsg) from None
        ...
        try:
            self._verify_units(
                self.trjfile.variables["time"].units, "picosecond"
            )
            self.has_time = True
        except KeyError:
            self.has_time = False
        ...
        self._verify_units(
            self.trjfile.variables["coordinates"].units, "angstrom"
        )
        ...
        for variable in self.trjfile.variables:
            if hasattr(self.trjfile.variables[variable], "scale_factor"):
                if variable in self.scale_factors:
                    scale_factor = self.trjfile.variables[
                        variable
                    ].scale_factor
                    ...
                    errmsg = (
                        "scale_factors for variable {0} are "
        ...
        if self.has_velocities:
            self._verify_units(
                self.trjfile.variables["velocities"].units,
                "angstrom/picosecond",
            )
        ...
        if self.has_forces:
            self._verify_units(
                self.trjfile.variables["forces"].units,
        ...
        if self.periodic:
            self._verify_units(
                self.trjfile.variables["cell_lengths"].units, "angstrom"
            )
    ...
    def _verify_units(eval_unit, expected_units):
        if eval_unit.decode("utf-8") != expected_units:
            errmsg = (
                "NETCDFReader currently assumes that the trajectory "
    ...
    def _read_frame(self, frame):
        ...
        if frame >= self.n_frames or frame < 0:
            raise IndexError(
                "frame index must be 0 <= frame < {0}".format(self.n_frames)
            )
...
class NCDFWriter(base.WriterBase):
    ...
    *without the NetCDF4 API and HDF5 bindings*. Since the conda version of
    ...
    def __init__(
        ...
        forces=False,
        scale_time=None,
        scale_cell_lengths=None,
        scale_cell_angles=None,
        scale_coordinates=None,
        scale_velocities=None,
        scale_forces=None,
        **kwargs,
    ):
        ...
        self.remarks = (
            remarks
            or "AMBER NetCDF format (MDAnalysis.coordinates.trj.NCDFWriter)"
        )
    ...
    def _init_netcdf(self, periodic=True):
        ...
        if netCDF4:
            ncfile = netCDF4.Dataset(
                self.filename, "w", format="NETCDF3_64BIT"
            )
        else:
            ncfile = scipy.io.netcdf_file(
                self.filename, mode="w", version=2, maskandscale=False
            )
            wmsg = (
                "Could not find netCDF4 module. Falling back to MUCH "
        ...
        ncfile.createDimension(
            "frame", None
        )  # unlimited number of steps (can append)
        ncfile.createDimension(
            "atom", self.n_atoms
        ...
        if self.periodic:
            ...
            cell_spatial = ncfile.createVariable(
            ...
            cell_angular = ncfile.createVariable(
    ...
    def _write_next_timestep(self, ts):
        ...
        if self.has_forces:
            forces = ts._forces
            if self.convert_units:
                forces = self.convert_forces_to_native(forces, inplace=False)
...
class NCDFPicklable(scipy.io.netcdf_file):
    ...
    def __setstate__(self, args):
        self.__init__(
            args[0], mmap=args[1], version=args[2], maskandscale=args[3]
        )
